// !A nivel de servidor es idéntico al resto, pero ese es nuestro server dominante para el merge!!!!
package main
...
func (s *server) AgregarBase(ctx context.Context, msg *pb.Crear) (*pb.CodigoRespuesta, error) {
	...
	escribirtxt("Tierra/"+msg.Sector, msg.Sector+" "+msg.Base+" "+msg.Valor)
	escribirlog("Tierra", "AgregarBase", msg.Sector, msg.Base, msg.Valor)
	relojVectorial[0] = relojVectorial[0] + 1
...
func (s *server) RenombrarBase(ctx context.Context, msg *pb.CambioNombre) (*pb.CodigoRespuesta, error) {
	...
	escribirlog("Tierra", "RenombrarBase", msg.Sector, msg.NombreOriginal, msg.NombreNuevo)
	relojVectorial[0] = relojVectorial[0] + 1
	//defer serv.Stop()
...
func (s *server) ActualizarValor(ctx context.Context, msg *pb.Actualizar) (*pb.CodigoRespuesta, error) {
	...
	escribirlog("Tierra", "ActualizarValor", msg.Sector, msg.Base, msg.NuevoValor)
	relojVectorial[0] = relojVectorial[0] + 1
	//defer serv.Stop()
	return &pb.CodigoRespuesta{Codigo: 1, Planeta: currentPlanet, RelojVectorial: relojVectorial}, nil //? nil== valor no inicializado, representa "no hay error", en este caso el retorno error==nil, por lo que todo estara bien si es que es eso lo que se retorna
}
func (s *server) BorrarBase(ctx context.Context, msg *pb.Borrar) (*pb.CodigoRespuesta, error) {
	...
	//if existe_base(msg.Sector,msg.Base){} Si existe, hacemos todo, sino existe, respondemos que no se pudo hacer nada xd
	escribirlog("Tierra", "RenombrarBase", msg.Sector, msg.Base, "")
...
func (s *server) SoldadosQuery(ctx context.Context, msg *pb.SoldadosRequest) (*pb.SoldadosAnswer, error) {
	...
	if _, err := os.Stat("Tierra/" + msg.Sector + ".txt"); err == nil {
		//! Si existe entramos aqui

		archivo, err := os.Open("Tierra/" + msg.Sector + ".txt")
		if err != nil {
			log.Fatal(err)
		}
		defer archivo.Close()
...
func (s *server) MergeRequest(ctx context.Context, msg *pb.Signal) (*pb.MergeAnswerString, error) {
	...
	archivo, err := os.Open("Tierra/log_Tierra.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer archivo.Close()
...
func (s *server) Consistencia(ctx context.Context, msg *pb.Consistency) (*pb.Ack, error) {
	...
	for linea := 0; linea < len(actualizacion); linea++ {
		...
		escribirtxt("Tierra/"+infoLinea[0], actualizacion[linea])
	}
...
func escribirtxt(path_archivo string, info_registro string) {
	file, err := os.OpenFile(path_archivo+".txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644) //!path sera donde se quiere escribir. ej Tierra/base.txt o Tierra/log_tierra.txt
	//!Tierra/archivo/base.txt?
...
func escribirlog(nombre_server_planeta string, nombre_accion string, nombre_sector string, nombre_base string, data_extra string) {
	...
	escribirtxt("Tierra/log_"+nombre_server_planeta, nombre_accion+" "+nombre_sector+" "+nombre_base+" "+data_extra) //! data_extra puede ser el nuevo nombre de la base, un valor extra de soldados, o vacio sin nada (depende de la accion que se vaya a hacer)
...
func update_base(accion string, nombre_sector string, nombre_base string, nuevo_valor string) bool { //! Le agregue la accion para ver caso x caso c:
	...
	f, err := os.Open("Tierra/" + nombre_sector + ".txt")
	if err != nil {
		return false
	}
	...
	file, err := os.OpenFile("Tierra/"+nombre_sector+"V2.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644) //path sera donde se quiere escribir. base.txt o puede ser Tierra/base.txt por ej <- depende como lo armemos
	if err != nil {
		panic(err)
	}
	data := bufio.NewWriter(file)
...
func change_namearch(nombre_sector string) {
	eliminarArchivoOriginal := os.Remove("Tierra/" + nombre_sector + ".txt") //! Esto lo elimina
	if eliminarArchivoOriginal != nil {
		log.Fatal(eliminarArchivoOriginal)
	}

	cambioNombre_ArchivoActualizado := os.Rename("Tierra/"+nombre_sector+"V2.txt", "Tierra/"+nombre_sector+".txt") //! Este le actualiza el nombre.
...
func main() {
	...
	listener, err := net.Listen("tcp", ":50052") // Listener conexion sincrona
	if err != nil {
